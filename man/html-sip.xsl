...
<xsl:template match="PARAM_NAME"><b><tt><xsl:value-of select="text()"/> </tt></b> </xsl:template>
...
<xsl:template match="DESCRIPTION_INDENT">
  ...
  <xsl:for-each  select="./DESCRIPTION_ITEM">
     <dt><b><xsl:value-of select="@label"/></b></dt><dd><xsl:apply-templates/></dd>
  </xsl:for-each>
  </dl>
</xsl:template>
...
<xsl:template match="ITEM">
	<li><b><font color="maroon"><xsl:value-of select="@label"/></font></b><xsl:apply-templates/>
	</li><br />
</xsl:template>
...
<xsl:template match="P">
	<p><xsl:apply-templates select="VERB|EM|BD|TT|A|LINK|text()"/></p>
</xsl:template>
